...
class TCP_Server_Socket(threading.Thread, metaclass=abc.ABCMeta):
    def __init__(self,socket,semaphore,username,Ulist):
    ...
    def run(self):
        ...
        print('Client closed.')
        self.Ulist.remove(self.username)
        self.socket.close()
...
class TCP_LRserver(TCP_Server_Socket,tcpCmdHandler):
    def __init__(self,socket,addr,locks,semaphore,username,Ulist):
        TCP_Server_Socket.__init__(self,socket,semaphore,username,Ulist)
        tcpCmdHandler.__init__(self,addr,locks,Ulist)
        self.username = username
        self.UID = "none"
...
class UDP_LRserver(UDP_Server_Socket,udpCmdHandler):
    def __init__(self,socket,locks):
        UDP_Server_Socket.__init__(self,socket)
        udpCmdHandler.__init__(self,locks)
...
def main(): 
    ...
    while True:
        ...
        totalCnt += 1
        username = f"user{totalCnt}"
        Users.append(username)
        TCP_LRserver(client_socket,client_addr,lock,semaphore,username,Users).start()
